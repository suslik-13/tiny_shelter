...
STAMINA_EXHAUST_DIG = 0.5		-- exhaustion increased this value after digged node
STAMINA_EXHAUST_PLACE = 0.1	-- .. after digging node
STAMINA_EXHAUST_MOVE = 0.5	-- .. if player movement detected
STAMINA_EXHAUST_JUMP = 2	-- .. if jumping
STAMINA_EXHAUST_CRAFT = 2	-- .. if player crafts
STAMINA_EXHAUST_PUNCH = 10	-- .. if player punches another player
...
SPRINT_SPEED = clamp(tonumber(minetest.settings:get("stamina_sprint_speed")) or 1.7, 0.0, 1.0)
-- how much higher player can jump if satiated
SPRINT_JUMP  = clamp(tonumber(minetest.settings:get("stamina_sprint_jump")) or 0.35, 0.0, 1.0)
-- how fast to drain satation while sprinting (0-1)
SPRINT_DRAIN  = clamp(tonumber(minetest.settings:get("stamina_sprint_drain")) or 0.55, 0.0, 1.0)
