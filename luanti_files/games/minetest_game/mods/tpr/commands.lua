...
minetest.register_chatcommand("tpy", {
	description = S(minetest.colorize("green", "Accept teleport requests from another player")),
	privs = {interact = true, tp = true},
	func = tp.tpr_accept
})

minetest.register_chatcommand("tpn", {
	description = S(minetest.colorize("red", "Deny teleport requests from another player")),
	privs = {interact = true, tp = true},
	func = tp.tpr_deny
})

minetest.register_chatcommand("tpr_mute", {
	description = S(minetest.colorize("orange", "Mutes a player: denies them from sending you teleport requests")),
	params = S("<playername> | leave playername empty to see help message"),
	privs = {interact = true, tp = true},
	func = tp.tpr_mute
})
...
minetest.register_chatcommand("tpr_unmute", {
	description = S(minetest.colorize("orange", "Unmutes a player: allow them to send you teleport requests again")),
	params = S("<playername> | leave playername empty to see help message"),
	privs = {interact = true, tp = true},
	func = tp.tpr_unmute
})
