...
function tp.tpr_mute(player, muted_player)
	...
	if not minetest.get_player_by_name(muted_player) then
		send_message(player, S(minetest.colorize("orange", "There is no player by that name. Keep in mind this is case-sensitive, and the player must be online.")))
		return
	end

	if minetest.check_player_privs(muted_player, {tp_admin = true}) and not minetest.check_player_privs(player, {server = true}) then
		send_message(player, S(minetest.colorize("orange", "tpr_mute: Failed to mute player @1: they have the tp_admin privilege."), muted_player))
		return
	end

	if muted_players[player] == muted_player then
		send_message(player, S(minetest.colorize("orange", "tpr_mute: Player @1 is already muted."), muted_player))
		return
	end

	muted_players[player] = muted_player
	send_message(player, S(minetest.colorize("orange", "tpr_mute: Player @1 successfully muted."), muted_player))
end

-- Unmutes a player from sending you teleport requests
function tp.tpr_unmute(player, muted_player)
	if muted_player == "" then
		send_message(player, S(minetest.colorize("orange", "Usage: /tpr_unmute <player>")))
		return
	end
	...
	if muted_players[player] ~= muted_player then
		send_message(player, S(minetest.colorize("orange", "tpr_mute: Player @1 is not muted yet."), muted_player))
		return
	end

	muted_players[player] = nil
	send_message(player, S(minetest.colorize("orange", "tpr_mute: Player @1 successfully unmuted."), muted_player))
end


-- custom start

function is_tp_protect(pos)
...
function tp.tpr_send(sender, receiver)
	...
	if muted_players[receiver] == sender and not minetest.check_player_privs(sender, {server = true}) then
		send_message(sender, S(minetest.colorize("red", "Cannot send request to @1 (you have been muted)."), receiver))
		return
	end

	if receiver == "" then
		send_message(sender, S(minetest.colorize("orange", "Usage: /tpr <Player name>")))
		return
	end

	if not minetest.get_player_by_name(receiver) then
		send_message(sender, S(minetest.colorize("orange", "There is no player by that name. Keep in mind this is case-sensitive, and the player must be online")))
		return
	end

	-- Spam prevention
	if spam_prevention[receiver] == sender and not minetest.check_player_privs(sender, {tp_admin = true}) then
		send_message(sender, S(minetest.colorize("orange", "Wait @1 seconds before you can send teleport requests to @2 again."), tp.timeout_delay, receiver))
		...
		minetest.after(tp.timeout_delay, function(sender_name, receiver_name)
			...
			if spam_prevention[receiver_name] == nil then
				send_message(sender_name, S(minetest.colorize("orange", "You can now send teleport requests to @1."), receiver_name))
				band = true
			end
		end, sender, receiver)
	...
	-- Compatibility with beerchat
		if minetest.get_modpath("beerchat") and not minetest.check_player_privs(sender, {tp_admin = true}) then
			...
			if player_receiver:get_meta():get_string("beerchat:muted:" .. sender) == "true" then
				send_message(sender, S(minetest.colorize("red", "You are not allowed to send requests because you're muted.")))
				return
			end
		end
		...
		if minetest.check_player_privs(sender, {tp_admin = true}) and tp.enable_immediate_teleport then
			...
			send_message(sender, S(minetest.colorize("green", "You are teleporting to @1."), receiver))
			return
		end
		...
		if minetest.get_modpath("gamehub") then -- Compatibility with gamehub (UNTESTED)
			if gamehub.players[receiver] then
				send_message(sender, S(minetest.colorize("red", "Teleport request denied, player is in the gamehub!")))
				return
			end
		end
...
sender_player = minetest.get_player_by_name(sender)

--if sender_player == nil then
--	return
--end

	position = sender_player:get_pos()
	if is_tp_protect(position) then
		send_message(sender, S(minetest.colorize("red", "Cannot send request to @1 (in this territory the request is prohibited)."), receiver))
		return
	end
-- custom end	

		send_message(receiver, S(minetest.colorize("orange", "@1 is requesting to teleport to you. /tpy to accept."), sender))
		send_message(sender, S(minetest.colorize("orange", "Teleport request sent! It will timeout in @1 seconds."), tp.timeout_delay))
...
function tp.tphr_send(sender, receiver)
	...
	if receiver == "" then
		send_message(sender, S(minetest.colorize("orange", "Usage: /tphr <Player name>")))
		return
	end
	...
	-- Compatibility with beerchat
		...
		if minetest.check_player_privs(sender, {tp_admin = true}) and tp.enable_immediate_teleport then
			...
			tp.tpr_accept(receiver)
			send_message(sender, S(minetest.colorize("green", "@1 is teleporting to you."), receiver))
			return
		end
...
receiver_player = minetest.get_player_by_name(receiver)

--if receiver_player == nil then
--	return
--end

	position = receiver_player:get_pos()
	if is_tp_protect(position) then
		send_message(sender, S(minetest.colorize("red", "Cannot send request to @1 (a player in the territory prohibited for this request)."), receiver))
		return
	end
-- custom end

		send_message(receiver, S(minetest.colorize("orange", "@1 is requesting that you teleport to them. /tpy to accept; /tpn to deny."), sender))
		send_message(sender, S(minetest.colorize("orange", "Teleport request sent! It will timeout in @1 seconds."), tp.timeout_delay))
		...
		minetest.after(tp.timeout_delay, function(sender_name, receiver_name)
			if tp.tphr_list[receiver_name] and tp.tpn_list[sender_name] then
				...
				send_message(sender_name, S(minetest.colorize("orange", "Request timed-out.")))
				send_message(receiver_name, S(minetest.colorize("orange", "Request timed-out.")))
				return
			end
		end, sender, receiver)
	end
...
function tp.tpc_send(sender, coordinates)
	...
	if posx==nil or posy==nil or posz==nil or string.len(posx) > 6 or string.len(posy) > 6 or string.len(posz) > 6 then
		send_message(sender, S(minetest.colorize("orange", "Usage: /tpc <x, y, z>")))
		return nil
	end
	...
	if tp.can_teleport(target_coords) == false then
		send_message(sender, S(minetest.colorize("red", "You cannot teleport to a location outside the map!")))
		return nil
	end
	...
	if minetest.check_player_privs(pname, {tp_admin = true}) then
		...
		target_coords = nil
		send_message(sender, S(minetest.colorize("orange", "Teleporting to: @1, @2, @3"), posx, posy, posz))
	else
		if minetest.check_player_privs(pname, {tp_tpc = true}) then
				...
				if minetest.get_modpath("areas") then
					for _, area in pairs(areas:getAreasAtPos(target_coords)) do
						if minetest.get_player_by_name(area.owner) then -- Check if area owners are online
							...
							send_message(sender, S(minetest.colorize("orange", "Area request sent! Waiting for @1 to accept your request.") ..
			...
			tp.tpc_teleport_player(sender)
			target_coords = nil
			send_message(sender, S(minetest.colorize("geen", "Teleporting to: @1, @2, @3"), posx, posy, posz))
		else
			send_message(sender, S(minetest.colorize("red", "Error: You do not have permission to teleport to those coordinates.")))
			return
		end
	end
end
...
function tp.tpr_deny(name)
	...
	and not tp.tpc_list[name] and not tp.tpn_list[name] then
		send_message(name, S(minetest.colorize("orange", "Usage: /tpn allows you to deny teleport/area requests sent to you by other players.")))
		return
	end
	...
	if tp.tpc_list[name] then
		...
		send_message(name2, S(minetest.colorize("red", "Area request denied.")))
		send_message(name, S(minetest.colorize("red", "You denied the request @1 sent you."), name2))
	...
	if tp.tpr_list[name] then
		...
		send_message(name2, S(minetest.colorize("red", "Teleport request denied.")))
		send_message(name, S(minetest.colorize("red", "You denied the request @1 sent you."), name2))
...
function tp.tpr_accept(name)
	...
	and not tp.tpc_list[name] then
		send_message(name, S(minetest.colorize("orange", "Usage: /tpy allows you to accept teleport/area requests sent to you by other players.")))
		return
	end
	...
	if tp.tpc_list[name] then
		...
		if tp.tpc_list[name] then
			...
			chatmsg = S(minetest.colorize("green", "@1 is teleporting to your protected area @2."), name2, minetest.pos_to_string(tpc_target_coords[name]))
			tp.tpc_list[name] = nil
		else
			return
		end
		...
		if not source or not target then
			send_message(name, S(minetest.colorize("orange", "@1 is not online right now."), name2))
			tp.tpc_list[name] = nil
			return
		end
		...
		if not tpc_target_coords[name] then
			...
			chatmsg = S(minetest.colorize("green", "@1 is teleporting to your protected area @2."), name2, minetest.pos_to_string(tpc_target_coords[name2]))
		else
		...
		send_message(name2, S(minetest.colorize("green", "Request Accepted!")))
	...
	if tp.tpr_list[name] then
		...
		source = minetest.get_player_by_name(name)
		target = minetest.get_player_by_name(name2)
		chatmsg = S(minetest.colorize("green", "@1 is teleporting to you."), name2)
		tp.tpr_list[name] = nil
	...
	elseif tp.tphr_list[name] then
		...
		chatmsg = S(minetest.colorize("green", "You are teleporting to @1."), name2)
		tp.tphr_list[name] = nil
	else
		return
	end
...
function tp.tpj(player, param)
	if param == "" then
		send_message(player, S(minetest.colorize("orange", "Usage: <x|y|z> <number>")))
		return false
	end
	...
	if not tonumber(args[2]) then
		send_message(player, S(minetest.colorize("red", "Not a number!")))
		return false, S(minetest.colorize("red", "Not a number!"))
	end
	...
	elseif args[1] == "z" then
		...
		send_message(player, S(minetest.colorize("red", "Not a valid axis. Valid options are X, Y or Z")))
		return
	end

	if tp.can_teleport(target_coords) == false then
		send_message(player, S(minetest.colorize("red", "You cannot teleport to a location outside the map!")))
		return
	end
...
if tp.enable_tpp_command then
	function tp.tpp(player, param)
		...
		elseif tp.available_places[param] then
			...
			send_message(player, S(minetest.colorize("green", "Teleporting to @1."), param))
