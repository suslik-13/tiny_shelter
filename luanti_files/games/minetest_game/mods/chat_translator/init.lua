...
function send_message(params)
    ...
    http.fetch(request, function(response)
        if not response.completed then
            minetest.log("error", "Chat Translator: HTTP request not completed")
            return
        end
        
        -- Check HTTP status code
        if response.code and response.code ~= 200 then
            minetest.log("error", "Chat Translator: HTTP error " .. response.code .. ": " .. (response.data or "no response data"))
            return
        end
        
        local msg = params.message
        local data_json = minetest.parse_json(response.data)
        if data_json and data_json.translatedText then
            msg = data_json.translatedText
        else
            handle_failure("Failed to translate a message. HTTP response: " .. (response.code or "unknown"))
            -- If translation failed, use original message
            msg = params.message
        end
        -- Ensure msg is not nil before concatenation
        if msg then
            minetest.chat_send_player(
                params.receiver_name,
                params.prefix ..
